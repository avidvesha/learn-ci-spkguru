...
class Rangking extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->page->setTitle('Hasil Rangking');
    }

    public function index()
    {
        $guru = $this->MGuru->getAll();
    ...
    {
        $nilai = $this->MNilai->getNilaiGuru();

        $dataInput = array();
        $no = 0;
        foreach ($guru as $item => $itemGuru) {
            foreach ($nilai as $index => $itemNilai) {
                if ($itemGuru->kdGuru == $itemNilai->kdGuru) {
                    $dataInput[$no]['guru'] = $itemGuru->guru;
